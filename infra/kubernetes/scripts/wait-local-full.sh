...
echo "[*] Vault mesh host bridge"
kubectl_cmd -n "$NS" get svc/vault-1 endpoints/vault-1 2>/dev/null || true

echo "[+] local-full workloads are ready or reported by rollout status as complete."
echo "[+] Settlement path: vault mesh (compose lab); mpc-sidecar / HashiCorp local-vault are not part of deploy."
