...
infra/kubernetes/scripts/deploy.sh production --dry-run
infra/kubernetes/scripts/deploy.sh production
```

## Reset stateless instances

```bash
infra/kubernetes/scripts/reset-instance.sh kerosene-production server rollout
infra/kubernetes/scripts/reset-instance.sh kerosene-production kfe-service rollout
infra/kubernetes/scripts/reset-instance.sh kerosene-production server pods
```

## Reset stateful instance without deleting data

```bash
infra/kubernetes/scripts/reset-instance.sh kerosene-production mpc-sidecar one-pod
```
...
```bash
infra/kubernetes/scripts/debug-pod.sh kerosene-production server --shell
```
...
```bash
infra/kubernetes/scripts/debug-pod.sh kerosene-production server --logs
...
```bash
infra/kubernetes/scripts/collect-diagnostics.sh kerosene-production
```
...
```bash
infra/kubernetes/scripts/rollback.sh kerosene-production server
infra/kubernetes/scripts/rollback.sh kerosene-production kfe-service
infra/kubernetes/scripts/rollback.sh kerosene-production server 3
```
...
`base/dependency-services/services.yaml` creates stable Service names for existing stateful workloads. This keeps the backend app deployable through the Kustomize stack while Postgres, Redis, Vault, Bitcoin Core and LND are migrated to dedicated operator-backed manifests.
