...
: > "$LOG_FILE"
CALL_LOG="$LOG_FILE" HOME="$TMP_DIR/no-kube-home" KUBECONFIG= KEROSENE_HOST_HOME="$TMP_DIR/no-kube-home" KEROSENE_LOG_FOLLOW=0 KUBECTL="$TMP_DIR/kubectl" bash "$TMP_DIR/scripts/logs.sh" --tail 20 >/dev/null
...
: > "$LOG_FILE"
CALL_LOG="$LOG_FILE" HOME="$TMP_DIR/no-kube-home" KUBECONFIG= KEROSENE_HOST_HOME="$TMP_DIR/no-kube-home" KEROSENE_LOG_RECONNECT=0 KUBECTL="$TMP_DIR/kubectl" bash "$TMP_DIR/scripts/logs.sh" server --tail 6 >/dev/null
...
: > "$LOG_FILE"
CALL_LOG="$LOG_FILE" HOME="$TMP_DIR/no-kube-home" KUBECONFIG= KEROSENE_HOST_HOME="$TMP_DIR/no-kube-home" KEROSENE_LOG_RECONNECT=0 KUBECTL="$TMP_DIR/kubectl" bash "$TMP_DIR/scripts/logs.sh" --follow server --tail 5 >/dev/null
...
CALL_LOG="$LOG_FILE" HOME="$TMP_DIR/no-kube-home" KUBECONFIG= KEROSENE_HOST_HOME="$TMP_DIR/no-kube-home" KEROSENE_LOG_RECONNECT=0 KUBECTL="$TMP_DIR/kubectl" bash "$TMP_DIR/scripts/logs.sh" --follow --split --output-dir "$SPLIT_DIR" --tail 7 >/dev/null
...
: > "$LOG_FILE"
status_output="$(CALL_LOG="$LOG_FILE" HOME="$TMP_DIR/no-kube-home" KUBECONFIG= KEROSENE_HOST_HOME="$TMP_DIR/no-kube-home" KUBECTL="$TMP_DIR/kubectl" MISSING_NS=1 bash "$TMP_DIR/scripts/status.sh" 2>&1)" || fail "status should not fail when namespace is missing"
grep -qF "Namespace kerosene-local does not exist" <<<"$status_output" || fail "status should explain missing namespace"

: > "$LOG_FILE"
status_output="$(CALL_LOG="$LOG_FILE" HOME="$TMP_DIR/no-kube-home" KUBECONFIG= KEROSENE_HOST_HOME="$TMP_DIR/no-kube-home" KUBECTL="$TMP_DIR/kubectl" bash "$TMP_DIR/scripts/status.sh" 2>&1)" || fail "status should succeed when namespace exists"
